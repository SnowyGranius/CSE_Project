...
from classes_cnn_800 import NoPoolCNN1
import time
...
for df in data_csv_blobs:
    permeability_values_blobs.extend(df['Permeability'])

# print(f'Number of permeability values: {len(permeability_values_blobs)}')

permeability_values = np.array(permeability_values_blobs)
...
test_permeability = permeability_values_blobs
test_images = data_images_blobs
train_permeability = permeability_values_blobs
train_images = data_images_blobs
# print(len(train_permeability))
# print(len(train_images))
# print(len(test_permeability))
...
batch_size = 1
trainloader = torch.utils.data.DataLoader(dataset_train, batch_size=batch_size, shuffle=False, num_workers=0)
...
for cnn in [NoPoolCNN1().to(my_device)]:
    ...
    # summary(NoPoolCNN1().to(my_device), (1, image_size, image_size))
    # torch.set_default_dtype(torch.float64)
    ...
    for lr in lr_list:
        ...
        ax.scatter(test_targets, test_predictions, alpha=0.6, color='blue')
