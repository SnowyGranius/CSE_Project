...
from classes_cnn_1000 import NoPoolCNN11, NoPoolCNN12, NoPoolCNN13
import time
...
csv_directory = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(sys.argv[0])))), 'Porespy_homogenous_diamater')
...
image_directory = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(sys.argv[0])))), f'Image_dataset_generation/Converge-study-{image_size}')
...
for cnn in [NoPoolCNN11().to(my_device), NoPoolCNN12().to(my_device), NoPoolCNN13().to(my_device)]:
    ...
    for lr in lr_list:
            ...
            if validation:
                ...
                with torch.no_grad():
                    for val_data in torch.utils.data.DataLoader(dataset_val, batch_size=batch_size, shuffle=False, num_workers=0):
                        ...
                        val_R_squared = r2_score(val_targets.cpu().numpy(), val_outputs.cpu().numpy())
        ...
        os.makedirs(os.path.join(os.path.dirname(os.path.abspath(sys.argv[0])), f'Pure-{image_size}-Evolution'), exist_ok=True)
        plt.savefig(os.path.join(os.path.dirname(os.path.abspath(sys.argv[0])), f'Pure-{image_size}-Evolution/Loss_R_squared-{cnn.__class__.__name__}-{lr:.0e}.png'))
        ...
        os.makedirs(os.path.join(os.path.dirname(os.path.abspath(sys.argv[0])), f'Pure-{image_size}-Log_results'), exist_ok=True)
        plt.savefig(os.path.join(os.path.dirname(os.path.abspath(sys.argv[0])), f'Pure-{image_size}-Log_results/Truth_vs_Predicted-test-logarithmic-{cnn.__class__.__name__}-{lr:.0e}.png'))
        ...
        ax.legend(loc='lower right')
        ax.text(0.05, 0.95, f'Test R^2: {R_squared_test:.5f}\nMSE Loss: {mse_loss_test}', transform=ax.transAxes, fontsize=14, verticalalignment='top')
        os.makedirs(os.path.join(os.path.dirname(os.path.abspath(sys.argv[0])), f'Pure-{image_size}-Normal_results'), exist_ok=True)
        plt.savefig(os.path.join(os.path.dirname(os.path.abspath(sys.argv[0])), f'Pure-{image_size}-Normal_results/Truth_vs_Predicted-test-{cnn.__class__.__name__}-{lr:.0e}.png'))
        plt.close('all')
