...
from classes_cnn_conv import NoPoolCNN1, NoPoolCNN3, NoPoolCNN4
import time
...
for cnn in [NoPoolCNN1().to(my_device), NoPoolCNN3().to(my_device), NoPoolCNN4().to(my_device)]:
    ...
    for lr in [1e-5, 5e-5, 1e-4, 5e-4, 1e-3, 5e-3]:
        ...
        os.makedirs(os.path.join(os.path.dirname(os.path.abspath(sys.argv[0])), f'Convergence-{image_size}-Evolution'), exist_ok=True)
        plt.savefig(os.path.join(os.path.dirname(os.path.abspath(sys.argv[0])), f'Convergence-{image_size}-Evolution/Loss_R_squared-{cnn.__class__.__name__}-{lr}.png'))
        ...
        os.makedirs(os.path.join(os.path.dirname(os.path.abspath(sys.argv[0])), f'Convergence-{image_size}-Log_results'), exist_ok=True)
        plt.savefig(os.path.join(os.path.dirname(os.path.abspath(sys.argv[0])), f'Convergence-{image_size}-Log_results/Truth_vs_Predicted-test-logarithmic-{cnn.__class__.__name__}-{lr}.png'))
        ...
        ax.legend(loc='lower right')
        ax.text(0.05, 0.95, f'Test R^2: {R_squared_test:.5f}', transform=ax.transAxes, fontsize=14, verticalalignment='top')
        os.makedirs(os.path.join(os.path.dirname(os.path.abspath(sys.argv[0])), f'Convergence-{image_size}-Normal_results'), exist_ok=True)
        plt.savefig(os.path.join(os.path.dirname(os.path.abspath(sys.argv[0])), f'Convergence-{image_size}-Normal_results/Truth_vs_Predicted-test-{cnn.__class__.__name__}-{lr}.png'))
        plt.close('all')
